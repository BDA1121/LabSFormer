...
class ShadowFormer(nn.Module):
    def __init__(self, img_size=256, in_chans=3,
                 embed_dim=32, depths=[2, 2, 2, 2, 2, 2, 2, 2, 2,2], num_heads=[1, 2, 4, 8, 16, 16, 8, 4, 2,1],
        ...
        self.num_enc_layers = len(depths)//2 +1
        self.num_dec_layers = len(depths)//2 + 1
        ...
        conv_dpr = [drop_path_rate]*depths[5]
        dec_dpr = enc_dpr[::-1]
        ...
        # Bottleneck
        self.conv = BasicShadowFormer(dim=embed_dim*16,
        ...
        self.decoderlayer_1 = BasicShadowFormer(dim=embed_dim*8,
                            output_dim=embed_dim*8,
        ...
        self.decoderlayer_2 = BasicShadowFormer(dim=embed_dim*4,
                            output_dim=embed_dim*4,
        ...
        self.decoderlayer_3 = BasicShadowFormer(dim=embed_dim*2,
                            output_dim=embed_dim*2,
    ...
    def forward(self, x, xm, mask=None):
        ...
        # print(f"pool0 shape: {pool0.shape}")
        ...
        # New Encoder Block
        conv3 = self.encoderlayer_3(pool2, xm3, mask=mask, img_size=self.img_size)
        ...
        # Bottleneck
        conv4 = self.conv(pool3, xm4, mask=mask, img_size=self.img_size)
        ...
        # print(f"deconv0 before layer shape: {deconv0.shape}")
        deconv0 = self.decoderlayer_0(deconv0, xm3, mask=mask, img_size=self.img_size)
        # print(f"deconv0 after layer shape: {deconv0.shape}")
        ...
        # print(f"deconv1 before layer shape: {deconv1.shape}")
        deconv1 = self.decoderlayer_1(deconv1, xm2, mask=mask, img_size=self.img_size)
        # print(f"deconv1 after layer shape: {deconv1.shape}")
        # # New Decoder Block
        ...
        # print(f"deconv2 before layer shape: {deconv2.shape}")

        deconv2 = self.decoderlayer_2(deconv2, xm1, mask=mask, img_size=self.img_size)
        # print(f"deconv2 after layer shape: {deconv2.shape}")
        ...
        # print(f"deconv3 before layer shape: {deconv3.shape}")

        deconv3 = self.decoderlayer_3(deconv3, xm, mask=mask, img_size=self.img_size)
        # print(f"deconv3 after layer shape: {deconv3.shape}")
