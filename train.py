...
from losses import CharbonnierLoss,LABLoss,PerceptualLoss,CosineSimilarityLoss,CombinedLoss
            ...
            with torch.no_grad():
                ...
                for ii, data_val in enumerate((val_loader), 0):
                    ...
                    with torch.cuda.amp.autocast():
                        restored = model_restoration(input_, mask)
                    # restored = torch.clamp(restored,0,1)
                    # restored[:, 0, :, :] = torch.clamp(restored[:, 0, :, :], 0, 1)  # L channel [0, 1]
                    # restored[:, 1, :, :] = torch.clamp(restored[:, 1, :, :], -1, 1)  # A channel [-1, 1]
                    # restored[:, 2, :, :] = torch.clamp(restored[:, 2, :, :], -1, 1)
