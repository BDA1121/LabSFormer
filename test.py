...
parser.add_argument('--result_dir', default='./results/11DLCC/',
...
with torch.no_grad():
    ...
    for ii, data_test in enumerate(tqdm(test_loader), 0):
        ...
        # rgb_restored[:, 0, :, :] = torch.clamp(rgb_restored[:, 0, :, :], 0, 1)  # L channel [0, 1]
        # rgb_restored[:, 1, :, :] = torch.clamp(rgb_restored[:, 1, :, :], -1, 1)  # A channel [-1, 1]
        # rgb_restored[:, 2, :, :] = torch.clamp(rgb_restored[:, 2, :, :], -1, 1)
        # Unpad the output
        ...
        if args.save_images:
            utils.save_img(rgb_restored, os.path.join(args.result_dir, filenames[0]))
